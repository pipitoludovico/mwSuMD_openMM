...
class mwInputParser:
    ...
    def checkEngine(self):
        self.initialParameters['MDEngine'] = 'GROMACS' if any(
            ...
            file.endswith('.psf') for file in os.listdir('./system')) else 'AMBER'
        self.getSystem()

    def getSystem(self):
        for ext in self.fileExtensions:
            for file in os.listdir('./system'):
                if file.endswith(ext) and not file.startswith('lig'):
    ...
    def getSettings(self):
        ...
        return self.initialParameters, self.selection_list
